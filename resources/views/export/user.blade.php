                        ...
                        ->when(isset($request->is_active), function ($query) use ($request) {
                            if (request()->boolean('is_active')) {
                                return $query->whereDoesntHave('disabled', function ($q) {
                                    $q->whereIn('disabled_setting_leave_types.business_id', [
...
<body>
            ...
            <tbody>
                @if (1)
                    @foreach ($leave_types as $key => $leave_type)
                        @php
                            $total_recorded_hours = \App\Models\LeaveRecord::whereHas('leave', function ($query) use (
                                $user,
                                $leave_type,
                            ) {
                                $query->where([
                                ...
                                ->get()
                                ->sum(function ($record) {
                                    return \Carbon\Carbon::parse($record->end_time)->diffInHours(
                ...
                @if (count($user->documents))
                    @foreach ($user->documents as $index => $document)
                        <tr class="table_row">
                            ...
                            @endif
                            @if (!empty($request->document_added_by))
                                <td>{{ $document->creator->first_Name . ' ' . $document->creator->last_Name . ' ' . $document->creator->middle_Name }}
                                </td>
                            @endif
                        </tr>
                    @endforeach
    ...
    @if (!empty($request->educational_history))
        <table>
            @if (count($user->education_histories))
                <thead>
                    <tr class="table_head_row">
                        <th class="index_col">Educational History</th>
                    </tr>
                    <tr class="table_head_row">
                        <th class="index_col">#</th>
    ...
    @if (!empty($request->current_passport_details))
        <table>
            @if (!empty($user->passport_detail))
                <thead>
                    <tr class="table_head_row">
                        <th>Current Passport Details</th>
                    </tr>
                    <tr class="table_head_row">
                        <th class="index_col">#</th>
    ...
    @if (!empty($request->current_right_to_works))
        <table>
            @if (!empty($user->right_to_work))
                <thead>
                    <tr class="table_head_row">
                        <th>Current Right To Works</th>
                    </tr>
                    <tr class="table_head_row">
    ...
    @if (!empty($request->contact_details))
        <table>
            @if (count($user->emergency_contact_details))
                <thead>
                    <tr class="table_head_row">
                        <th class="index_col">Contact Details</th>
                    </tr>
                    <tr class="table_head_row">
                        <th class="index_col">#</th>
    ...
    @if (!empty($request->bank_details))
        <table>
            @if (!empty($user->bank_details_name))
                <thead>
                    <tr class="table_head_row">
                        <th class="index_col">Bank Details</th>
                    </tr>
                    <tr class="table_head_row">
                        <th class="index_col">#</th>
    ...
    @if (!empty($request->social_links))
        <table>
            @if (count($user->social_links))
                <thead>
                    <tr class="table_head_row">
                        <th class="index_col">Social Links</th>
                    </tr>
                    <tr class="table_head_row">
                        <th class="index_col">#</th>
